...
export type PnlCardData = {
  ...
  exitPx: number;
  size: number;
  /** Montant $ risqué au SL */
  riskedUsd: number;
  /** Capital sortie = risqué + profit net */
...
/** Charge ordres historiques + levier puis construit la carte. */
...
export function pnlCardFilename(data: PnlCardData): string {
  ...
  return `LVDC-${displaySymbol(data.coin)}-${data.side}-${date}.png`;
}
